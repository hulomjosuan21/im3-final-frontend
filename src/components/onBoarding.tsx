...
const WelcomeStep: React.FC = () => {
    ...
    return (
        <div className="w-screen h-screen flex justify-center items-center flex-col">
            {View}
            <div className="h-24">
                <TypingAnimation duration={25} className="text-sm font-lg">What is your primary field of study?</TypingAnimation>
            </div>
            <div className="flex justify-center gap-2 flex-wrap mx-4 max-w-2xl">
                {
                    fields.map((field, index) => (
                        <Badge key={index} variant={'outline'} className="p-2 rounded-full cursor-pointer hover:scale-105 hover:opacity-90 transition ease-in-out">{field}</Badge>
                    ))
                }
            </div>
        </div>
...
const ProfileStep: React.FC = () => {
    ...
    return (
        <div className="w-screen h-screen flex items-center justify-center">{View}</div>
    )
}
...
const FinishStep: React.FC = () => (
    <div className="w-screen h-screen grid place-content-center text-4xl font-bold">Wapa homan</div>
);
